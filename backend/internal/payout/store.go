...
const accountColumns = `creator_id,stripe_account_id,transfers_status,charges_enabled,payouts_enabled,details_submitted,requirements_due,created_at,updated_at`

func scanAccount(row pgx.Row) (Account, error) {
	var value Account
	err := row.Scan(&value.CreatorID, &value.StripeAccountID, &value.TransfersStatus, &value.ChargesEnabled, &value.PayoutsEnabled, &value.DetailsSubmitted, &value.RequirementsDue, &value.CreatedAt, &value.UpdatedAt)
	return value, err
}
...
func (r *PostgresRepository) Upsert(ctx context.Context, creatorID string, stripeAccount StripeAccount, now time.Time) (Account, error) {
	value, err := scanAccount(r.pool.QueryRow(ctx, `
		INSERT INTO creator_payout_accounts
			(creator_id,stripe_account_id,transfers_status,charges_enabled,payouts_enabled,details_submitted,requirements_due,created_at,updated_at)
		...
		RETURNING `+accountColumns, creatorID, stripeAccount.ID, stripeAccount.TransfersStatus, stripeAccount.ChargesEnabled, stripeAccount.PayoutsEnabled, stripeAccount.DetailsSubmitted, stripeAccount.RequirementsDue, now))
	if err != nil {
